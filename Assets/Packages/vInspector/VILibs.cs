...
namespace VInspector.Libs
{
    public static class VUtils
    {
        ...
        public static string FormatLong(long l) => $"{l:n0}";
        ...
        public static string FormatSize(long bytes, bool sizeUnknownIfNotMoreThanZero = false)
        {
            ...
            if (bytes < 0) return "? B";
            if (i < 3) return $"{bytes:0.#} " + ss[i];
            return $"{bytes:0.##} " + ss[i];
        }
        public static string FormatTime(long ms, bool includeMs = false)
        {
            var t = System.TimeSpan.FromMilliseconds(ms);
            var s = "";
        ...
        private static string CountSuffix(long c) => c % 10 != 1 ? "s" : "";
        ...
        private static Dictionary<Type, Dictionary<int, MethodInfo>> methodInfoCache = new Dictionary<Type, Dictionary<int, MethodInfo>>();
        ...
        public static Color HSLToRGB(float h, float s, float l)
        {
            ...
            var r = hue2Rgb(k2, k1, h + 1f / 3);
            var g = hue2Rgb(k2, k1, h);
            var b = hue2Rgb(k2, k1, h - 1f / 3);
    ...
    public static class VGUI
    {
        ...
        private static FieldInfo _fi_s_Current = typeof(Event).GetField("s_Current", maxBindingFlags);
        ...
        private static Stack<float> _indentLabelWidthStack = new Stack<float>();
